...
=> fix
The issue here is the method call to #send; that method is not defined in
PostalService or its parent class, so the method being invoked is
Object#send, which interprets the first argument as a method invocation.

The method #send is defined in the module Mailing, which is not included in
PostalService. We can fix the error by including the module Mailing in that
class.

It is not a great idea to name a method #send because it overrides the
Object#send method as demonstrated here.
