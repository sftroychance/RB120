...
=> fix
The attr_reader for @serial_number in Employee is set to private, so the
method call `other.serial_number` will not work, since `other` is an object
external to the class. If we set the attr_reader for @serial_number to
protected, this will still prevent access to the variable outside the class
but will allow the method to be called on other objects from within the
Employee class (as in `Employee#==`).
=end
